...
def inspect_json(source: Path, *extra: str) -> list[Any]:
    results = run_public_cli("inspect", str(source), "--profile", "x4-crosspoint", "--json", *extra)
...
def test_unreadable_source_refuses_without_reaching_a_canonical_document(tmp_path: Path) -> None:
    ...
    results = run_public_cli("inspect", str(missing), "--profile", "x4-crosspoint", "--json")
