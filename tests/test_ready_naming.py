...
from tests.ready_fixtures import BODY, COMPLETED, PROFILE, facts, inbox_note, ready_reports, report
from tests.workspace_fixtures import workspace_environment

READY_ARGUMENTS = ("--profile", PROFILE, "--ready", "--json")


def _entry_point_contexts(root: Path) -> list[tuple[list[str], Path, dict[str, str]]]:
    """Give each installed entry point an independent Workspace and environment."""

    contexts: list[tuple[list[str], Path, dict[str, str]]] = []
    for index, command in enumerate(public_cli_commands()):
        workspace = root / f"workspace-{index}"
        environment = workspace_environment(workspace, root / f"home-{index}")
        contexts.append((command, workspace, environment))
    return contexts


def _prepare(
    command: Sequence[str], source: Path, environment: dict[str, str]
) -> subprocess.CompletedProcess[str]:
    """Prepare one source as Ready through one chosen installed entry point."""

    return run_command(command, "prepare", str(source), *READY_ARGUMENTS, environment=environment)


def test_ready_name_preserves_ordinary_title_spaces(tmp_path: Path) -> None:
    """A Ready Artifact looks like the book title through both installed entry points."""

    source = inbox_note(
        tmp_path,
        body="---\ntitle: A Plain Book\n---\n\nA short body with enough words to prepare.\n",
    )
    for command, workspace, environment in _entry_point_contexts(tmp_path):
        result = _prepare(command, source, environment)
...
def test_ready_name_preserves_unicode_and_portable_punctuation(tmp_path: Path) -> None:
    ...
    for command, _workspace, environment in _entry_point_contexts(tmp_path):
        result = _prepare(command, source, environment)
        assert result.returncode == COMPLETED
        published = Path(str(facts(report(result.stdout), "artifact")["path"]))
...
def test_colliding_ready_name_reserves_its_digest_inside_the_byte_budget(
    ...
    for command, _workspace, environment in _entry_point_contexts(tmp_path):
        first = _prepare(command, first_source, environment)
        assert first.returncode == COMPLETED
        original = Path(str(facts(report(first.stdout), "artifact")["path"]))
        original_bytes = original.read_bytes()
        second = _prepare(command, second_source, environment)
...
def test_ready_name_turns_unsafe_characters_into_normalised_separators(tmp_path: Path) -> None:
    ...
    result = _prepare(public_cli_commands()[0], source, environment)
    assert result.returncode == COMPLETED
    published = Path(str(facts(report(result.stdout), "artifact")["path"]))
    ...
    slash_result = _prepare(public_cli_commands()[0], slash_source, environment)
...
def test_identical_bytes_are_reused_with_their_own_evidence(tmp_path: Path) -> None:
    """Both entry points reuse the artifact without rewriting and retain separate Reports."""
    ...
    for command, workspace, environment in _entry_point_contexts(tmp_path):
        ...
        second = _prepare(command, copied, environment)
        assert second.returncode == COMPLETED
