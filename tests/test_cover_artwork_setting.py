...
def _validate(tmp_path: Path, body: str) -> list[tuple[int, dict[str, object]]]:
    ...
    return [
        (result.returncode, command_document(result))
...
def test_an_omitted_cover_artwork_key_is_off_by_default(tmp_path: Path) -> None:
    for code, document in _validate(tmp_path, "version = 1\n\n" + INBOX):
...
def test_an_explicit_cover_artwork_boolean_is_configured(tmp_path: Path, stated: bool) -> None:
    ...
    for code, document in _validate(tmp_path, body):
        assert code == COMPLETED
...
def test_a_non_boolean_cover_artwork_value_is_invalid(tmp_path: Path, line: str) -> None:
    for code, document in _validate(tmp_path, f"version = 1\n{line}\n\n{INBOX}"):
        assert code == REFUSED
...
def test_an_unknown_key_is_still_refused_and_version_stays_one(tmp_path: Path) -> None:
    ...
    for code, document in _validate(
        tmp_path / "unknown", 'version = 1\ncolour = "blue"\n\n' + INBOX
    ):
        assert code == REFUSED
