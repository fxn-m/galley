...
from tests.support.epub_fixtures import PACKAGE_PATH, default_entries, without, write_epub
...
def test_human_audit_output_describes_the_same_result(tmp_path: Path) -> None:
    ...
    result = run_cli("audit", str(book), "--profile", "x4-crosspoint", environment=NO_EPUBCHECK)
...
def test_audit_with_an_unknown_profile_refuses_without_reading_the_subject(tmp_path: Path) -> None:
    ...
    result = run_cli("audit", str(book), "--profile", "missing", "--json", environment=NO_EPUBCHECK)
