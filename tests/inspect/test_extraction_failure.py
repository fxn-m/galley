...
from tests.support.public_cli import NO_DEFUDDLE, run_cli
...
def refuse(source: str) -> Any:
    result = run_cli("inspect", source, "--profile", "x4-crosspoint", "--json")
    assert (result.returncode, result.stderr) == (3, "")
    return json.loads(result.stdout)


def complete(source: str) -> Any:
    result = run_cli("inspect", source, "--profile", "x4-crosspoint", "--json")
