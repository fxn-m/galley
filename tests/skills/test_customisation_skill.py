...
from tests.support.workspace_fixtures import command_document, field, valid_workspace


def test_installed_customisation_guide_is_reachable_and_its_example_validates(
    tmp_path: Path,
) -> None:
    target = tmp_path / "skills-0"
    environment = isolated_home(tmp_path / "home-0")
    installed = run_cli(
        "skill", "install", "--target", str(target), "--json", environment=environment
    )
    ...
    workspace = tmp_path / "workspace-0"
