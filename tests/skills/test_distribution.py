...
from tests.skills.skill_fixtures import (
    MANIFEST,
    SKILLS,
    contents,
    document_of,
...
from tests.support.markdown_fixtures import RETAINED_EVIDENCE_BASELINE, native_ast, write_markdown
...
def test_both_distributions_carry_complete_skills_and_cover_font(tmp_path: Path) -> None:
    ...
    for skill in SKILLS:
        for relative in packaged_files(skill):
            ...
            assert f"galley-0.1.11/src/galley/skills/{skill}/{relative}" in sdist
    for asset in (
        "AtkinsonHyperlegible-Regular.otf",
        "AtkinsonHyperlegible-OFL.txt",
    ):
        assert f"galley/assets/fonts/{asset}" in wheel
        assert f"galley-0.1.11/src/galley/assets/fonts/{asset}" in sdist
